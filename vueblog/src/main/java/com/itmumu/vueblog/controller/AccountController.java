...
import com.itmumu.vueblog.common.dto.LoginDto;
...
import com.itmumu.vueblog.util.JwtUtils;
...
@RestController public class AccountController {
    ...
    public Result login(@Validated @RequestBody LoginDto loginDto, HttpServletResponse response) {
        ...
        return Result.succ(MapUtil.builder().put("id", user.getId()).put("username", user.getUsername())
            .put("avatar", user.getAvatar()).put("email", user.getEmail()).map());
    }

    @RequiresAuthentication @GetMapping("/logout") public Result logout() {
